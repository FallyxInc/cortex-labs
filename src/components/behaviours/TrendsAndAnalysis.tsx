...
// Define AI insights type locally since it's complex
interface AIInsightsType {
...
export default function TrendsAndAnalysis({
  name,
  altName,
  data,
  getTimeOfDay,
  ...
  const [viewMode, setViewMode] = useState<"holistic" | "personalized">(
    "holistic",
  );
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      const residents = [
        ...new Set(
          data
            .map((item: BehaviourIncident) => item.name)
  ...
  useEffect(() => {
    ...
    const dataToAnalyze =
      viewMode === "personalized" && selectedResident
      ...
      try {
        ...
        const response = await fetch("/api/trends/ai-insights", {
          ...
          body: JSON.stringify({
            ...
            viewMode,
            residentName:
              viewMode === "personalized" ? selectedResident : undefined,
          }),
        });
  ...
  return (
    <div style={{ padding: "24px", maxWidth: "1400px", margin: "0 auto" }}>
      ...
      >
        ...
        >
          <strong>Active Development:</strong> This section is under ongoing
          improvement. Features and functionality may change as we continue to
        ...
        >
          <button
            ...
            style={{
              ...
              border: "none",
              backgroundColor:
                viewMode === "holistic" ? "#3b82f6" : "transparent",
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              backgroundColor:
                viewMode === "personalized" ? "#3b82f6" : "transparent",
      ...
      >
        <SummaryCard
          ...
          trend={
            analysisData.trends.overallTrend as
              | "increasing"
              | "decreasing"
              | "stable"
          }
                    ...
                    generateLabels: function (chart) {
                      ...
                      if (
                        ...
                        return data.labels.map((label, i) => {
                          ...
                          return {
                            text: String(label), // Use full label without truncation
      ...
      {loadingAI ? (
        ...
        <>
          ...
          {aiInsights.clinicalAssessment &&
            !aiInsights.clinicalAssessment
              .toLowerCase()
              .includes("unable to generate") &&
            !aiInsights.clinicalAssessment
              .toLowerCase()
              .includes("unable to assess") && (
              <div
                style={{
                  backgroundColor: "white",
                  borderRadius: "8px",
          ...
          {aiInsights.carePlanSuggestions &&
            aiInsights.carePlanSuggestions.length > 0 && (
              <div
                style={{
                  backgroundColor: "white",
                  borderRadius: "8px",
  ...
  return (
    ...
    >
      ...
      {trend && (
        ...
        >
          <TrendIcon style={{ display: "inline-block", marginRight: "4px" }} />{" "}
          {trend === "increasing"
            ? "Increasing"
            : trend === "decreasing"
              ? "Decreasing"
...
function TableCard({
  ...
  title: string;
  data: Array<{ label: string; count: number | unknown }>;
}) {
  return (
    <div
      style={{
...
function isValidAIInsights(insights: AIInsightsType | null): boolean {
  if (!insights) return false;

  const hasValidAssessment = Boolean(
    ...
    !insights.clinicalAssessment
      .toLowerCase()
      .includes("please review the data manually"),
  );
  ...
  const hasValidCarePlan = Boolean(
    insights.carePlanSuggestions && insights.carePlanSuggestions.length > 0,
  );

  const hasValidRiskFactors = Boolean(
    insights.riskFactors && insights.riskFactors.length > 0,
  );

  const hasValidEffectiveness = Boolean(
    insights.interventionEffectiveness &&
    !insights.interventionEffectiveness
      .toLowerCase()
      .includes("unable to generate") &&
    !insights.interventionEffectiveness
...
function calculateAnalysis(
  ...
  const dateTimestamps = dates.map((d) => d.getTime());
  ...
  data.forEach((item) => {
    if (item.time) {
      ...
      if (timeOfDay in timeOfDayCounts) {
        timeOfDayCounts[timeOfDay]++;
      }
    }
  });
  const sortedTimeOfDay = Object.entries(timeOfDayCounts).sort(
    ([, a], [, b]) => b - a,
  );
  ...
  const personalizedPatterns =
    residents.length === 1
      ? generatePersonalizedPatterns(data, getTimeOfDay)
      : null;
  ...
  if (outcomesWithInterventions.length > 0) {
    const positiveOutcomes = outcomesWithInterventions.filter((item: BehaviourIncident) => {
      ...
      return (
        ...
        outcome.includes("improved") ||
        outcome.includes("success")
      );
    }).length;
    const effectivenessRate =
      (positiveOutcomes / outcomesWithInterventions.length) * 100;
    ...
    : [
        {
          ...
          description:
            "Review the charts and patterns above for detailed insights into behaviour trends.",
          severity: "low",
        },
      ];
}
