...
export default function BehavioursReports({
  name,
  altName,
  data,
  getTimeOfDay,
  ...
  const chartHoverCountsRef = useRef<Record<string, number>>({});
  const chartHoverStartTimesRef = useRef<Record<string, number>>({});
  const exportClickCountRef = useRef<number>(0);
  const [selectedHome, setSelectedHome] = useState<string>(name);
  ...
  const [availableResidents, setAvailableResidents] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      const units = [
        ...new Set(
          data
            .map((item) => item.room || item.homeUnit)
            .filter((x): x is string => Boolean(x)),
        ),
      ];
      const residents = [
        ...new Set(
          data.map((item) => item.name).filter((x): x is string => Boolean(x)),
        ),
      ];
    ...
    try {
      ...
      if (selectedResidents.length > 0) {
        filteredData = filteredData.filter(
          (item) => item.name && selectedResidents.includes(item.name),
        );
      }
      ...
      const allReports: Record<string, ReportData> = {};
  ...
  const generateReport = (
    ...
    const residentsAffected = new Set(
      filteredData.map((item) => item.name).filter(Boolean),
    ).size;
    const unitsInvolved = new Set(
      filteredData.map((item) => item.room || item.homeUnit).filter(Boolean),
    ).size;

    let chartLabels: string[] = [];
    let chartValues: number[] = [];
    let tableData: ReportTableRow[] = [];
    ...
    switch (type) {
      case "timeOfDay":
        ...
        tableData = chartLabels.map((label, idx) => ({
          ...
          percentage:
            totalEvents > 0
              ? ((chartValues[idx] / totalEvents) * 100).toFixed(1)
              : "0.0",
        }));
        break;
      ...
      case "behaviourType":
        ...
        filteredData.forEach((item) => {
          const itemType =
            item.incident_type || item.behaviour_type || "Unknown";
      ...
      case "behaviourBreakdown":
        ...
        filteredData.forEach((item) => {
          const category =
            item.behaviour_type || item.incident_type || "Unknown";
          const subtype =
            item.behaviour_subtype || item.behaviourCategory || "";
          ...
          key = cleanDuplicatedText(key) || key;
          breakdownCounts[key] = (breakdownCounts[key] || 0) + 1;
        });
        const sortedBreakdown = Object.entries(breakdownCounts).sort(
          ([, a], [, b]) => (b as number) - (a as number),
        );
        chartLabels = sortedBreakdown.map(
          ([label]) => cleanDuplicatedText(label) || label,
        );
        chartValues = sortedBreakdown.map(([, count]) => count as number);
        tableData = chartLabels.map((label, idx) => ({
          label,
          count: chartValues[idx],
          percentage:
      ...
      case "resident":
        ...
        chartValues = sortedResidents.map(([, count]) => count as number);
        tableData = chartLabels.map((label, idx) => ({
          label,
          count: chartValues[idx],
          percentage:
      ...
      case "unit":
        ...
        chartValues = sortedUnits.map(([, count]) => count as number);
        tableData = chartLabels.map((label, idx) => ({
          label,
          count: chartValues[idx],
          percentage:
      ...
      case "dayOfWeek":
        ...
        chartValues = chartLabels.map(
          (day) => dayOfWeekCounts[day as DayOfWeek] || 0,
        );
        tableData = chartLabels.map((label, idx) => ({
          label,
          count: chartValues[idx],
  ...
  const handleExportPDF = async () => {
    if (!reportContentRef.current || Object.keys(allReportsData).length === 0)
      return;
    ...
    pdf.text("Behaviours Analysis Report", pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition = 80;
    ...
    if (firstReport) {
      ...
      pdf.setTextColor(0, 0, 0);
      pdf.text(
        firstReport.summary.totalEvents.toString(),
        startX + 15,
        yPosition + 50,
      );
      ...
      pdf.text(
        "Residents Affected",
        startX + cardWidth + cardSpacing + 15,
        yPosition + 20,
      );
      pdf.setFontSize(24);
      pdf.setFont(fontName, "bold");
      pdf.setTextColor(0, 0, 0);
      pdf.text(
        firstReport.summary.residentsAffected.toString(),
        startX + cardWidth + cardSpacing + 15,
        yPosition + 50,
      );
      ...
      pdf.setTextColor(0, 0, 0);
      pdf.text(
        firstReport.summary.unitsInvolved.toString(),
        startX + 15,
        yPosition + 50,
      );
    ...
    for (const reportType of reportTypes) {
      ...
      const estimatedSpace =
        30 + 300 + 30 + (reportData.table?.length || 0) * 20;
      ...
      if (reportData.table && reportData.table.length > 0) {
        ...
        if (typeof pdf.autoTable === "function") {
          ...
          const percentageWidth = tableWidth * 0.2; // 20% for Percentage
          ...
          const tableEndY = pdf.lastAutoTable
            ? pdf.lastAutoTable.finalY
            : yPosition + tableData.length * 20 + 30;
          pdf.setDrawColor(200, 200, 200);
          pdf.setLineWidth(1);
          pdf.roundedRect(
            tableX - 5,
          ...
          const percentageWidth = tableWidth * 0.2;
          ...
          pdf.text(
            "Count",
            tableX + categoryWidth + countWidth / 2 - 10,
            currentY + 12,
            { align: "center" },
          );
          pdf.text(
            "Percentage",
            tableX + categoryWidth + countWidth + percentageWidth / 2 - 15,
            currentY + 12,
            { align: "center" },
          );
          currentY += cellHeight;
          ...
          tableData.forEach((row, idx) => {
            ...
            pdf.text(
              row[1],
              tableX + categoryWidth + countWidth / 2,
              currentY + 12,
              { align: "center" },
            );
            ...
            pdf.text(
              row[2],
              tableX + categoryWidth + countWidth + percentageWidth / 2,
              currentY + 12,
              { align: "center" },
            );
            ...
            >
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  clickCountsRef.current[key] =
                    (clickCountsRef.current[key] || 0) + 1;
                  trackReportsPageClick({
        ...
        >
          ...
          {!loading && Object.keys(allReportsData).length > 0 && (
            <>
              ...
              {/* Render all reports */}
              {Object.entries(allReportsData).map(
                  ...
                  const chartOptions = {
                    onHover: (
                      ...
                      if (activeElements.length > 0) {
                        const chartId = `${reportType}_chart`;
                        ...
                        chartHoverCountsRef.current[chartId] =
                          (chartHoverCountsRef.current[chartId] || 0) + 1;

                        trackChartInteraction({
                          chartType: "bar",
                          chartName: `${reportType} - chart`,
                    ...
                    onLeave: (
                      ...
                      if (chartHoverStartTimesRef.current[chartId]) {
                        const hoverDuration =
                          Date.now() - chartHoverStartTimesRef.current[chartId];
                        trackChartInteraction({
                          chartType: "bar",
                          chartName: `${reportType} - chart`,
                          interactionType: "hover",
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          Behaviours by{" "}
                          {getReportTypeLabel(reportType).toLowerCase()}
                        </p>
                        ...
                        >
                          <div
                            ref={(el) => {
                              if (el) chartRefs.current[reportType] = el;
                            }}
                            style={{
                              width: "100%",
                              height: "400px",
                              ...
                              >
                                ...
                                <tbody>
                                  {reportData.table.map((row, index) => (
                                    <tr
                                      key={`${reportType}-${row.label}-${index}`}
                                    >
                                      <td>{row.label}</td>
                                      <td>{row.count}</td>
