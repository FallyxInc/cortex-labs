...
import { BehaviourIncident } from "@/types/behaviourTypes";
...
export default function BeTrackingTable({
  filteredData,
  cleanDuplicateText,
  storageKey = "behaviours_checked_items",
}: TrackingTableProps) {
  ...
  const handleRadioChange = (incidentNumber: string | undefined): void => {
  ...
  const toggleExpanded = (incidentNumber: string | undefined): void => {
    if (!incidentNumber) return;
        ...
        <tbody id="fallsTableBody">
          {filteredData &&
            filteredData.map((item, i) => {
              ...
              const triggersText =
                cleanDuplicateText(item.triggers, "triggers") || "";
              const interventionsText =
                cleanDuplicateText(item.interventions, "interventions") || "";

              const summaryHasCompliance =
                summaryText.includes("No Progress") &&
                summaryText.includes("24hrs of RIM");
              const triggersHasCompliance =
                triggersText.includes("No Progress") &&
                triggersText.includes("24hrs of RIM");
              const interventionsHasCompliance =
                interventionsText.includes("No Progress") &&
                interventionsText.includes("24hrs of RIM");

              // Strikethrough style for selected rows
              const strikethroughStyle: React.CSSProperties = isSelected
              ...
              return (
                <tr key={i} style={s.tableRow}>
                  ...
                  <td style={{ ...s.tableCell, ...strikethroughStyle }}>
                  ...
                  <td
                    style={{
                      ...s.tableCell,
                      ...(summaryHasCompliance ? s.highlightRed : {}),
                      ...strikethroughStyle,
                    }}
                  >
                    {item.summary}
                  ...
                  <td
                    style={{
                      ...s.tableCell,
                      ...(triggersHasCompliance ? s.highlightRed : {}),
                      ...strikethroughStyle,
                    }}
                  >
                    {triggersText}
                  ...
                  <td
                    style={{
                      ...s.tableCell,
                      ...(interventionsHasCompliance ? s.highlightRed : {}),
                      ...strikethroughStyle,
                    }}
                  >
                    {interventionsText}
                  ...
                  <td
                    style={{
                      ...s.tableCell,
                      ...(!hasOtherNotes ? s.lastCell : {}),
                      ...strikethroughStyle,
                    }}
                  >
                    {item.CI || "Still Gathering Data/Unknown"}
                  </td>
                  {hasOtherNotes && (
                    <td
                      style={{
                    ...
                    >
                      {item.other_notes
                        ? (() => {
                            ...
                            const expanded =
                              expandedNotes[item.incident_number || ""] ||
                              false;
                            const maxLength = 200;
